...
public class Tab {
    ...
    public void PrintProductions() {
        ...
        for (int i = 0; i < nonterminals.size(); i++) {
            ...
            trace.Write(" ");
            Visit(s.graph, false, false);
            trace.WriteLine();
            visited.clear();
        }
    }
    
    private void Visit(Node n, boolean fragileContext, boolean insideAlternation) {
        while (n != null && !visited.get(n.n)) {
            visited.set(n.n);

            switch (n.typ) {
            case Node.t:
            case Node.nt:
                trace.Write(n.sym.name + " ");
                if (!n.up) {
                    Visit(n.next, true, false);
                }
                fragileContext = true;
                break;
            case Node.wt:
                trace.Write("WEAK " + n.sym.name + " ");
                fragileContext = true;
                break;
            case Node.clas:
            case Node.chr:
                // character class & character never show up in productions
                //trace.Write("" + classes.get(n.val));
                fragileContext = true;
                break;
            case Node.alt:
                boolean needsParens = fragileContext;
                
                if (!insideAlternation) {
                    Node t = n.next;
           
                    if (t != null && t.next != null && !t.up) {
                        needsParens = true;
                    }
                }
                if (needsParens) {
                ...
                Visit(n.sub, true, false);

                if (n.down != null) {
                    trace.Write("| ");
                    Visit(n.down, false, true);
                } 
                if (needsParens) {
                    trace.Write(") ");
                }
	...
	public void SetDDT (String s) {
		...
		for (int i = 0; i < s.length(); i++) {
			...
			if ('0' <= ch && ch <= '9') ddt[ch - '0'] = true;
				...
				case 'R' : ddt[9] = true; break; // print production rules
				default : break;
			}
		}
	}
